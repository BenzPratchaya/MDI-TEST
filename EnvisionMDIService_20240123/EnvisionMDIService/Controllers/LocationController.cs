...
{
    public class LocationController : IeController
    {
        private const string controller_name = "Location";
        ...
        public LocationController() : base(ServiceConfig.EnableTraceInbound) { }
        ...
        public object SelectView([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                DataSet ds_mdi = mdi_db.SelectDataSet(SqlScriptName.Location_SelectView, json_obj_request);

                if (Utilities.HasData(ds_mdi))
                {
                    foreach (DataTable dtt_mdi in ds_mdi.Tables)
                        dtt_mdi.TableName = dtt_mdi.TableName.Replace("Location_SelectView", controller_name);
        ...
        public object SelectByUid([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                DataSet ds_mdi = mdi_db.SelectDataSet(SqlScriptName.Location_SelectByUid, json_obj_request);

                if (Utilities.HasData(ds_mdi))
                {
                    foreach (DataTable dtt_mdi in ds_mdi.Tables)
                        dtt_mdi.TableName = dtt_mdi.TableName.Replace("Location_SelectByUid", controller_name);
        ...
        public object Insert([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                mdi_db.Execute(SqlScriptName.Location_Insert, json_obj_request);

                json_obj_request["LocationId"] = mdi_db.LastInsertedId;
                json_obj_response.Add(mdi_db.Ack);
        ...
        public object Update([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                mdi_db.Execute(SqlScriptName.Location_Update, json_obj_request);
