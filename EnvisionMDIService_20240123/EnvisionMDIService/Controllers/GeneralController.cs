...
{
    public class GeneralController : IeController
    {
        private const string controller_name = "General";
        ...
        public GeneralController() : base(ServiceConfig.EnableTraceInbound) { }
        ...
        public object SelectView([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                DataSet ds_mdi = mdi_db.SelectDataSet(SqlScriptName.General_SelectView, json_obj_request);

                if (Utilities.HasData(ds_mdi))
                {
                    foreach (DataTable dtt_mdi in ds_mdi.Tables)
                        dtt_mdi.TableName = dtt_mdi.TableName.Replace("General_SelectView", controller_name);
        ...
        [HttpPost]
        [ActionName("SelectViewByGroup")]
        public object SelectViewByGroup([FromBody] dynamic raw)
        {
            string function_name = $"{controller_name}_SelectViewByGroup";
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                DataSet ds_mdi = mdi_db.SelectDataSet(SqlScriptName.General_SelectViewByGroup, json_obj_request);

                if (Utilities.HasData(ds_mdi))
                {
                    foreach (DataTable dtt_mdi in ds_mdi.Tables)
                        dtt_mdi.TableName = dtt_mdi.TableName.Replace("General_SelectViewByGroup", controller_name);
        ...
        public object Insert([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                mdi_db.Execute(SqlScriptName.General_Insert, json_obj_request);

                json_obj_request["GeneralId"] = mdi_db.LastInsertedId;
                json_obj_response.Add(mdi_db.Ack);
        ...
        public object Update([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                mdi_db.Execute(SqlScriptName.General_Update, json_obj_request);
