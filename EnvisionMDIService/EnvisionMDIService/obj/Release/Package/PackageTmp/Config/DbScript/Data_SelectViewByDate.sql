select
	...
	`patient_visit`.`fall_risk_type` as FallRiskType,
	`patient_visit`.`wh_dt` as `WhDateTime`,
	`patient_visit`.`created_on` as `CreatedOn`,
	`gbl_smoking_status`.`general_name` as `SmokingStatusText`,
	`gbl_coma_e_score`.`general_name` as `ComaEScoreText`,
	`gbl_coma_v_score`.`general_name` as `ComaVScoreText`,
	`gbl_coma_m_score`.`general_name` as `ComaMScoreText`,
	`gbl_fall_risk_type`.`general_name` as `FallRiskTypeText`,
...
from
  ...
  left join `mdi_modality` `modality_wh`
	...
	left join `gbl_general` `gbl_smoking_status` 
			on `gbl_smoking_status`.general_uid = `patient_visit`.`smoking_status` and `gbl_smoking_status`.general_group = 'SMOKING_STATUS'
	left join `gbl_general` `gbl_coma_e_score` 
			on `gbl_coma_e_score`.general_uid = `patient_visit`.`coma_e_score` and `gbl_coma_e_score`.general_group = 'COMA_E_SCALE'
	left join `gbl_general` `gbl_coma_v_score` 
			on `gbl_coma_v_score`.general_uid = `patient_visit`.`coma_v_score` and `gbl_coma_v_score`.general_group = 'COMA_V_SCALE'
	left join `gbl_general` `gbl_coma_m_score` 
			on `gbl_coma_m_score`.general_uid = `patient_visit`.`coma_m_score` and `gbl_coma_m_score`.general_group = 'COMA_M_SCALE'
	left join `gbl_general` gbl_fall_risk_type
			on `gbl_fall_risk_type`.general_id = `patient_visit`.`fall_risk_type` and `gbl_fall_risk_type`.general_group = 'FALL_RISK_TYPE'
where
