...
namespace EnvisionMDIService.Controllers
{
    public class DataController : IeController
    {
        ...
        */
        ...
        [HttpPost]
        [ActionName("SelectByDateGroupByModality")]
        public object SelectByDateGroupByModality([FromBody] dynamic raw)
        {
            string function_name = $"{controller_name}_SelectByDateGroupByModality";
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                DataSet ds_mdi = mdi_db.SelectDataSet(SqlScriptName.Data_SelectByDateGroupByModality, json_obj_request);

                if (Utilities.HasData(ds_mdi))
                {
                    foreach (DataTable dtt_mdi in ds_mdi.Tables)
                        dtt_mdi.TableName = dtt_mdi.TableName.Replace("Data_SelectByDateGroupByModality", controller_name);
