...
{
    public class UserController : IeController
    {
        ...
        public UserController() : base(ServiceConfig.EnableTraceInbound) { }

        [HttpPost]
        [ActionName("SelectLogin")]
        public object SelectLogin([FromBody] dynamic raw)
        {
            string function_name = $"{controller_name}_SelectLogin";
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                DataSet ds_mdi = mdi_db.SelectDataSet(SqlScriptName.User_SelectLogin, json_obj_request);

                if (Utilities.HasData(ds_mdi))
                {
                    foreach (DataTable dtt_mdi in ds_mdi.Tables)
                        dtt_mdi.TableName = dtt_mdi.TableName.Replace("User_SelectLogin", controller_name);
        ...
        public object Insert([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                mdi_db.Execute(SqlScriptName.User_Insert, json_obj_request);

                json_obj_request["UserId"] = mdi_db.LastInsertedId;
                json_obj_response.Add(mdi_db.Ack);
        ...
        public object Update([FromBody] dynamic raw)
        {
            ...
            Func<string, dynamic> func = delegate (string rawText)
            {
                ...
                mdi_db.Parameters = null;
                mdi_db.Execute(SqlScriptName.User_Update, json_obj_request);
